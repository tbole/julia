...
ioslength(io::IOBuffer) = (io.seekable ? io.size : bytesleft(io))
...
let bstream = BufferStream()
    ...
    @test bytesleft(bstream) == 0
    @test sprint(show, bstream) == "BufferStream() bytes waiting:$(bytesleft(bstream.buffer)), isopen:true"
    a = rand(UInt8,10)
    write(bstream,a)
    @test !eof(bstream)
